...
cd ../export/$2
psql -d $1 -A -F", " -c "SELECT username, password, title AS role FROM users JOIN roles ON users.role_fk=roles.role_id" > users.csv
psql -d $1 -A -F", " -c "SELECT code AS fcode, name AS common_name FROM facilities" > facilities.csv
psql -d $1 -A -F", " -c "SELECT asset_tag, description, name AS facility, disposed FROM assets JOIN asset_at ON assets.asset_id=asset_at.asset_fk JOIN facilities ON facilities.facility_id=asset_at.facility_fk" > assets.csv
