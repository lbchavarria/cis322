...
psql -d $1 -c "INSERT INTO asset_at (asset_fk, facility_fk, acquired, disposed) SELECT asset_id, facility_id, acquired, disposed FROM assets JOIN temp ON assets.asset_tag=temp.asset_tag JOIN facilities ON facilities.name=temp.asset_facility_name WHERE temp.asset_tag IS NOT NULL"
psql -d $1 -c "INSERT INTO trans_request (requester_id, request_time, source, destination, asset_fk, approver_id, approval_time, load, unload) SELECT requester.user_id, request_time, source.facility_id, destination.facility_id, asset_id, approver.user_id, approval_time, load, unload FROM temp JOIN users requester ON requester.username=temp.requester_username JOIN facilities source ON source.name=temp.source_name JOIN facilities destination ON destination.name=temp.destination_name JOIN assets ON assets.asset_tag=temp.trans_asset_tag JOIN users approver ON approver.username=temp.approver_username WHERE trans_asset_tag IS NOT NULL"
